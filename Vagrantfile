...
Vagrant.configure(2) do |config|
    ...
    config.vm.network "private_network", ip: "10.40.199.254"

    config.vm.network "forwarded_port", guest: 80, host: 8081 # horizon
    config.vm.network "forwarded_port", guest: 5000, host: 5000
    config.vm.network "forwarded_port", guest: 8776, host: 8776 # cinder
    config.vm.network "forwarded_port", guest: 8774, host: 8774 # nova
    ...
    #     shell.inline = "sudo apt-get update && sudo apt-get install -f && sudo rm -rf /var/lib/apt/lists/* && sudo apt-get update"
    # end
    ...
    # Install terraform
    config.vm.provision "shell" do |shell|
        shell.inline = "cd ~ && /vagrant/scripts/terraform.sh"
